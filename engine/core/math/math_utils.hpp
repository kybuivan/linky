...
static inline auto compute_cofactor(glm::mat4 const& m) -> glm::mat4
{
    ...
    glm::vec4 fac_0(coef_00, coef_00, coef_02, coef_03);
    glm::vec4 fac_1(coef_04, coef_04, coef_06, coef_07);
    glm::vec4 fac_2(coef_08, coef_08, coef_10, coef_11);
    glm::vec4 fac_3(coef_12, coef_12, coef_14, coef_15);
    glm::vec4 fac_4(coef_16, coef_16, coef_18, coef_19);
    glm::vec4 fac_5(coef_20, coef_20, coef_22, coef_23);

    glm::vec4 vec_0(m[1][0], m[0][0], m[0][0], m[0][0]);
    glm::vec4 vec_1(m[1][1], m[0][1], m[0][1], m[0][1]);
    glm::vec4 vec_2(m[1][2], m[0][2], m[0][2], m[0][2]);
    glm::vec4 vec_3(m[1][3], m[0][3], m[0][3], m[0][3]);

    glm::vec4 inv_0(vec_1 * fac_0 - vec_2 * fac_1 + vec_3 * fac_2);
    glm::vec4 inv_1(vec_0 * fac_0 - vec_2 * fac_3 + vec_3 * fac_4);
    glm::vec4 inv_2(vec_0 * fac_1 - vec_1 * fac_3 + vec_3 * fac_5);
    glm::vec4 inv_3(vec_0 * fac_2 - vec_1 * fac_4 + vec_2 * fac_5);

    glm::vec4 sign_A(+1, -1, +1, -1);
    glm::vec4 sign_B(-1, +1, -1, +1);
    glm::mat4 inverse(inv_0 * sign_A, inv_1 * sign_B, inv_2 * sign_A, inv_3 * sign_B);
